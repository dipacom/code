 ...
 *    (X = pengaturan 'monev_reminder_interval_bulan', default 3).
...
if (!function_exists('processLuaranReminders')) {

function processLuaranReminders(PDO $pdo, int $user_id): array {
    $interval_bulan = max(1, (int)(getSetting($pdo, 'monev_reminder_interval_bulan') ?: 3));
    $now_ts = time();
    ...
    foreach ($proposals as $p) {
        ...
        if ($warn_ts <= $now_ts && $dl_luaran_ts >= $now_ts) {
            insertReminderIfMissing($pdo, $p, 0, date('Y-m-d', $warn_ts), 'final', $interval_bulan, $dl_luaran_ts);
        }
        ...
        if ($dl_luaran_ts < $now_ts) {
            insertReminderIfMissing($pdo, $p, -1, date('Y-m-d', $dl_luaran_ts), 'final', $interval_bulan, $dl_luaran_ts);
        }
    }
...
function insertReminderIfMissing(PDO $pdo, array $p, int $period_no, string $period_date, string $tipe, int $interval_bulan, int $dl_luaran_ts): void {
    try {
        $pdo->prepare("
 ...
 */
function markLuaranRemindersRead(PDO $pdo, int $user_id, array $reminder_ids = []): int {
