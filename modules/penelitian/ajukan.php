...
// Pengaturan
$deadline = getSetting($pdo,'penelitian_deadline') ?: '2026-05-18 16:00:00';
...
if (!$ada_buka) {
    ...
    redirect('/modules/penelitian/index.php');
}
...
$edit_data = null;
if ($edit_id) {
    $ed = $pdo->prepare("SELECT * FROM usulan_penelitian WHERE id=? AND user_id=? AND status='draft'");
    $ed->execute([$edit_id, $uid]);
    $edit_data = $ed->fetch();
    if (!$edit_data) { redirect('/modules/penelitian/index.php'); }
}
...
function prosesUploadDokumen($file, $allowed_exts, $allowed_mimes, $max_mb, $uid, $prefix, $id_lang) {
    ...
    $dir = BASE_PATH . '/uploads/proposal_penelitian/' . $uid . '/';
    if (!is_dir($dir)) mkdir($dir, 0755, true);
    $fname = time() . '_' . $prefix . preg_replace('/[^a-zA-Z0-9_.]/', '_', $file['name']);
    if (move_uploaded_file($file['tmp_name'], $dir . $fname)) return ['err' => '', 'path' => 'uploads/proposal_penelitian/' . $uid . '/' . $fname, 'name' => $file['name'], 'size' => $file['size']];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Validasi
    if (!$skema)  $error = $id?'Pilih skema penelitian.':'Please select a research scheme.';
        ...
        if ($status === 'diajukan') {
            ...
            foreach ($admins as $adm) {
                $pdo->prepare("INSERT INTO notifikasi (user_id, judul, pesan, tipe) VALUES (?,?,?,?)")
                    ->execute([
                        $adm['id'],
                        $id?'Usulan Penelitian Baru':'New Research Proposal',
                        ($id?'Dosen ':'Lecturer ').$_SESSION['nama'].
                        ($id?' mengajukan proposal penelitian: ':' submitted a research proposal: ').$judul,
                        'info',
                    ]);
            }
        }
    ...
    if ($error && $action === 'ajukan' && $skema) {
        ...
        try {
            ...
            redirect('/modules/penelitian/ajukan.php?edit=' . $saved_id);
        } catch (\Exception $e) {
    ...
    <div class="topbar">
      <div>
        <div class="topbar-title">
          <?= ic('upload') ?>
          <?= $edit_data?($id?'Edit Proposal':'Edit Proposal'):($id?'Ajukan Proposal Penelitian':'Submit Research Proposal') ?>
          <span class="breadcrumb"><?= $tahun ?></span>
        </div>
      </div>
      <div class="topbar-right">
        <a href="<?= BASE_URL ?>/modules/penelitian/index.php" class="btn btn-outline" style="font-size:12px">
          ← <?= $id?'Kembali':'Back' ?>
        </a>
        ...
        <div class="card" style="margin-bottom:18px;border-radius:12px;overflow:hidden;padding:0">
          ...
          <div class="sec-body">
            ...
            <div class="form-group" style="margin-bottom:18px">
              <label class="form-label"><?= $id?'Skema Penelitian':'Research Scheme' ?> <span class="required">*</span></label>
              ...
              <div style="margin-top:6px;font-size:11px;color:var(--text-muted)">
                <?= $id?'Skema dipilih dari halaman sebelumnya. <a href="'.BASE_URL.'/modules/penelitian/index.php" style="color:var(--primary)">Ganti skema</a>':'Scheme selected from previous page. <a href="'.BASE_URL.'/modules/penelitian/index.php" style="color:var(--primary)">Change scheme</a>' ?>
              </div>
            ...
            <div class="form-group" style="margin-bottom:14px">
              <label class="form-label"><?= $id?'Judul Penelitian':'Research Title' ?> <span class="required">*</span></label>
            ...
            <div class="form-group">
              ...
              <textarea name="abstrak" class="form-control" rows="4" style="resize:vertical"
                placeholder="<?= $id?'Ringkasan latar belakang, tujuan, dan metode penelitian (opsional)':'Summary of background, objectives, and research method (optional)' ?>"><?= htmlspecialchars($pre_abstrak) ?></textarea>
        ...
        <div class="card" style="margin-bottom:18px;border-radius:12px;overflow:hidden;padding:0">
          ...
          <div class="sec-body">
            ...
            <div class="req-box">
              ...
              <span><?= $id
                ? 'Nama file yang dikirimkan: <strong>NamaPeneliti-Proposal Penelitian 2026.docx</strong>. Batas similarity maks. <strong>25%</strong> dan AI maks. <strong>30%</strong>.'
        ...
        <div class="card" style="margin-bottom:20px;border-radius:12px;overflow:hidden;padding:0">
          ...
          <div class="sec-body">
            ...
            <div style="display:flex;gap:10px;align-items:center;flex-wrap:wrap">
              ...
              <a href="<?= BASE_URL ?>/modules/penelitian/index.php" class="btn btn-outline">
                ← <?= $id?'Batal':'Cancel' ?>
              </a>
            </div>
...
(function(){
  ...
  const BASE = <?= json_encode(BASE_URL . '/modules/penelitian/cek_anggota.php') ?>;
  const CACHE = {};
