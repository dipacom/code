...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'upload_laporan') {
    ...
    if ($pid) {
        ...
        $chk = $pdo->prepare("SELECT id,judul,status FROM usulan_penelitian WHERE id=? AND user_id=? AND deleted_at IS NULL");
        $chk->execute([$pid, $uid]);
        $prop = $chk->fetch();
        ...
        if (!$prop || !in_array($prop['status'], ['disetujui','penandatanganan_kontrak','kontrak_aktif','laporan_diterima','selesai'], true)) {
            ...
            redirect('/modules/penelitian/laporan.php');
        }

        // Cek sudah ada laporan sebelumnya yang diterima
        $existing = $pdo->prepare("SELECT id,status,round_ke FROM laporan_penelitian WHERE usulan_id=?");
        ...
        // Update / insert. JANGAN hapus file lama: tetap diarsipkan via laporan_revisi snapshot.
        if ($lp) {
            $next_round = (int)($lp['round_ke'] ?? 1) + ($lp['status'] === 'revisi' ? 1 : 0);
            $pdo->prepare("UPDATE laporan_penelitian SET file_laporan=?,file_laporan_name=?,file_laporan_size=?,tanggal_submit=NOW(),status='menunggu',round_ke=?,catatan_admin=NULL,reviewed_by=NULL,reviewed_at=NULL,updated_at=NOW() WHERE usulan_id=?")
                ->execute([$rel_path, $file['name'], $file['size'], $next_round, $pid]);
        } else {
            $pdo->prepare("INSERT INTO laporan_penelitian (usulan_id,user_id,file_laporan,file_laporan_name,file_laporan_size,tanggal_submit,status,round_ke) VALUES (?,?,?,?,?,NOW(),'menunggu',1)")
        ...
        foreach ($admin_ids as $aid) {
            $pdo->prepare("INSERT INTO notifikasi (user_id,judul,pesan,tipe) VALUES (?,?,?,?)")
                ->execute([$aid,
                    $id?'Laporan Penelitian Masuk':'Research Report Submitted',
                    ($id?'Laporan penelitian telah diunggah oleh peneliti untuk proposal: ':'Research report uploaded for proposal: ')
...
<title><?= $id?'Laporan Penelitian':'Research Report' ?> — LPPM</title>
  ...
  <div class="main-content">
    ...
    <div class="topbar">
      <div>
        <div class="topbar-title">
          ...
          <?= $id?'Laporan Penelitian':'Research Report' ?>
    ...
    <div class="page-content">
      ...
      <div class="info-box">
        ...
        <?= $id
          ? 'Halaman ini untuk mengunggah <strong>laporan akhir penelitian</strong> bagi proposal yang telah disetujui. Upload file PDF laporan lengkap (maks. 30 MB).'
      ...
      <div class="lap-card">
        ...
        <div style="margin-top:14px">
          <button onclick="openUpload(<?= $p['id'] ?>, '<?= htmlspecialchars(addslashes($p['judul']),ENT_QUOTES) ?>')"
            ...
            <?= $id?'Upload Laporan Penelitian':'Upload Research Report' ?>
          </button>
          <?php if (!$has_kontrak): ?>
          <div style="margin-top:8px;font-size:11.5px;color:var(--text-muted)">
            <?= $id?'Kontrak penelitian belum tersedia — Hubungi admin LPPM.':'Research contract not yet available — Contact LPPM admin.' ?>
          </div>
          <?php endif; ?>
        </div>
        <?php endif; ?>
...
<div id="uploadModal" style="display:none;position:fixed;inset:0;z-index:1000;background:rgba(0,0,0,.45);align-items:center;justify-content:center">
  <div style="background:var(--bg-card);border-radius:16px;width:100%;max-width:500px;margin:20px;box-shadow:0 20px 60px rgba(0,0,0,.3)">
    <div style="padding:20px 22px;border-bottom:1.5px solid var(--border);display:flex;justify-content:space-between;align-items:center">
      <div style="font-size:15px;font-weight:700"><?= $id?'Upload Laporan Penelitian':'Upload Research Report' ?></div>
