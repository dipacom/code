...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'submit_luaran') {
        ...
        $chk = $pdo->prepare("SELECT id FROM usulan_penelitian WHERE id=? AND user_id=? AND deleted_at IS NULL");
        $chk->execute([$pid, $uid]);
        if (!$chk->fetch()) {
            $_SESSION['flash'] = ['type'=>'danger','msg'=>'Proposal tidak valid.'];
            redirect('/modules/penelitian/luaran.php');
        }
        ...
        $p = $pdo->prepare("SELECT judul FROM usulan_penelitian WHERE id=?");
        $p->execute([$pid]); $pd = $p->fetch();
        foreach ($admin_ids as $aid) {
            $pdo->prepare("INSERT INTO notifikasi (user_id,judul,pesan,tipe) VALUES (?,?,?,?)")
                ->execute([$aid,
                    $id?'Luaran Penelitian Disubmit':'Research Output Submitted',
...
<title><?= $id?'Luaran Penelitian':'Research Output' ?> — LPPM</title>
    ...
    <div class="topbar">
      <div>
        <div class="topbar-title">
          <?= ic('award') ?>
          <?= $id?'Luaran Penelitian':'Research Output' ?>
        </div>
      </div>
    ...
    <div class="page-content">
      ...
      <div style="background:#eff6ff;border:1.5px solid #bfdbfe;border-radius:11px;padding:13px 16px;margin-bottom:16px;font-size:12.5px;color:#1e40af">
        ...
        <span style="margin-left:5px"><?= $id
          ? 'Halaman ini untuk <strong>menyerahkan bukti luaran penelitian</strong> (jurnal, prosiding, HKI, dll.) dari hibah yang telah Anda laksanakan. Batas waktu luaran bervariasi per skema penelitian. Lampirkan link / DOI dan file bukti (PDF/JPG).'
      ...
      <div style="padding:60px 20px;text-align:center;color:var(--text-muted)">
        ...
        <div style="margin-top:5px;font-size:12.5px">
          <?= $id?'Luaran hanya dapat disubmit setelah kontrak penelitian Anda aktif.':'Output can only be submitted after your contract is active.' ?>
        </div>
      </div>
  ...
  <div class="lua-box">
    <form method="POST" enctype="multipart/form-data">
      ...
      <div style="padding:16px 20px;border-bottom:1.5px solid var(--border);display:flex;justify-content:space-between;align-items:center">
        <div>
          <div style="font-size:14px;font-weight:700"><?= $id?'Submit Luaran Penelitian':'Submit Research Output' ?></div>
