...
$pid = (int)($_GET['pid'] ?? 0);
if (!$pid) { redirect('/modules/penelitian/index.php'); }
...
if (!$proposal) { redirect('/modules/penelitian/index.php'); }
...
$round_ke = (int)($pdo->prepare("SELECT COUNT(*) FROM revisi_proposal WHERE usulan_id=? AND tipe=?")
    ->execute([$pid, $tipe_revisi]) ? $pdo->query("SELECT COUNT(*) FROM revisi_proposal WHERE usulan_id=$pid AND tipe='$tipe_revisi'")->fetchColumn() : 0) + 1;
// More reliable round count
$rq = $pdo->prepare("SELECT COUNT(*) FROM revisi_proposal WHERE usulan_id=? AND tipe=?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'kirim_revisi') {
    ...
    if (!$error) {
        if (empty($_FILES['file_revisi']['name'])) {
            ...
            if (!in_array($ext, ['doc','docx']) || !in_array($mime, $allowed_mimes)) {
                ...
                $dir = BASE_PATH . '/uploads/revisi_penelitian/' . $uid . '/';
                if (!is_dir($dir)) mkdir($dir, 0755, true);
                $fname = time() . '_rev' . $round_ke . '_' . preg_replace('/[^a-zA-Z0-9_.]/', '_', $f['name']);
                if (move_uploaded_file($f['tmp_name'], $dir . $fname)) {
                    $file_path = 'uploads/revisi_penelitian/' . $uid . '/' . $fname;
                    $file_name = $f['name'];
                    $file_size = $f['size'];
                } else {
    ...
    if (!$error) {
        ...
        $pdo->prepare("UPDATE usulan_penelitian SET status=?, updated_at=NOW() WHERE id=?")
