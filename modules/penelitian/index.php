...
if (empty($skema_rows)) {
    $gl_buka = (getSetting($pdo,'penelitian_global_buka') ?: 'buka') === 'buka';
    $ns_buka = (getSetting($pdo,'penelitian_nasional_buka') ?: 'buka') === 'buka';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($act === 'delete' && $pid) {
        ...
        $row = $pdo->prepare("SELECT file_proposal,file_cek_mandiri FROM usulan_penelitian WHERE id=? AND user_id=? AND deleted_at IS NOT NULL");
        $row->execute([$pid, $uid]);
        $del = $row->fetch();
        if ($del) {
            ...
            $pdo->prepare("DELETE FROM usulan_penelitian WHERE id=? AND user_id=?")->execute([$pid, $uid]);
        }
...
if (empty($_SESSION['trash_purged_'.date('Ymd')])) {
    try {
        $old = $pdo->prepare("SELECT id,file_proposal,file_cek_mandiri FROM usulan_penelitian WHERE user_id=? AND deleted_at IS NOT NULL AND deleted_at < DATE_SUB(NOW(), INTERVAL 30 DAY)");
        $old->execute([$uid]);
        foreach ($old->fetchAll() as $oi) {
            foreach (['file_proposal','file_cek_mandiri'] as $col) {
                if (!empty($oi[$col]) && file_exists(BASE_PATH.'/'.$oi[$col])) unlink(BASE_PATH.'/'.$oi[$col]);
            }
            $pdo->prepare("DELETE FROM usulan_penelitian WHERE id=?")->execute([$oi['id']]);
        }
    } catch (\Exception $e) {}
...
if (!empty($proposals)) {
    ...
    try {
        $rq = $pdo->prepare("
            ...
            LEFT JOIN reviewer_penilaian rp
...
// ── Kontrak penelitian milik dosen ini (#4) ─────────────────────
$kontrak_map = [];
if (!empty($proposals)) {
    $prop_ids = array_column($proposals, 'id');
    $ph = implode(',', array_fill(0, count($prop_ids), '?'));
...
if (!empty($proposals)) {
    ...
    try {
        $rvq = $pdo->prepare("
            ...
            FROM revisi_proposal WHERE usulan_id IN ($ph)
...
<title><?= $id?'Usulan Penelitian':'Research Proposals' ?> — LPPM IAKN Toraja</title>
  ...
  <div class="main-content">
    ...
    <div class="page-content">
      ...
      <div class="rp-hero">
        <div class="rp-hero-inner">
          <div class="rp-hero-top">
            ...
            <div>
              <div class="rp-hero-title">
                <?= $id?'Penerimaan Proposal Penelitian':'Research Proposal Submission' ?> <?= $tahun ?>
              </div>
          ...
          <div class="rp-hero-desc">
            <?= $id
              ? 'LPPM IAKN Toraja membuka penerimaan proposal penelitian tahun anggaran 2026. Tersedia dua skema riset: Publikasi Bereputasi Global (Scopus Q1-3) dan Publikasi Bereputasi Nasional (SINTA 1-6).'
      ...
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px">
        ...
        <a href="<?= BASE_URL ?>/modules/penelitian/ajukan.php"
           class="btn btn-primary" style="display:inline-flex;align-items:center;gap:7px;font-size:13px">
      ...
      <div class="rp-card">
        <div class="rp-card-head">
          ...
          <div class="rp-card-right">
            ...
            <a href="<?= BASE_URL ?>/modules/penelitian/ajukan.php?edit=<?= $p['id'] ?>"
            ...
            <a href="<?= BASE_URL ?>/modules/penelitian/revisi.php?pid=<?= $p['id'] ?>"
        ...
        <div class="rev-panel">
          ...
          <?php // ── Kontrak penelitian (untuk dosen #4) ── ?>
            ...
            <div style="display:flex;align-items:center;gap:10px;margin-bottom:10px">
              ...
              <div style="flex:1">
                <div style="font-size:12.5px;font-weight:700;color:#4a1d96">
                  <?= $id?'Kontrak Penelitian':'Research Contract' ?>
                </div>
