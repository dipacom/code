...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($_POST['action'] === 'simpan_penilaian') {
        ...
        $asgn = $pdo->prepare("SELECT ra.*, up.id as prop_id FROM reviewer_assignment ra
            JOIN usulan_penelitian up ON up.id=ra.usulan_id
        ...
        if (!$asgn) {
            ...
            redirect('/modules/penelitian/reviewer.php');
        }

        // Aturan #2: penilaian yang sudah dikirimkan tidak dapat diubah
        $sub_check = $pdo->prepare("SELECT submitted_at FROM reviewer_penilaian WHERE assignment_id=?");
        ...
        if (!empty($existing_sub)) {
            ...
            redirect('/modules/penelitian/reviewer.php?pid='.$asgn['prop_id']);
        }
        ...
        if (!empty($_FILES['file_review']['name'])) {
            ...
            if (in_array($ext, ['pdf','doc','docx'])) {
                $dir = BASE_PATH . '/uploads/review_penelitian/' . $uid . '/';
                if (!is_dir($dir)) mkdir($dir, 0755, true);
                $fname = time() . '_review_' . $assign_id . '_' . preg_replace('/[^a-zA-Z0-9_.]/', '_', $f['name']);
                if (move_uploaded_file($f['tmp_name'], $dir . $fname)) {
                    $file_review      = 'uploads/review_penelitian/' . $uid . '/' . $fname;
        ...
        // Upsert penilaian
        $existing = $pdo->prepare("SELECT id, file_review FROM reviewer_penilaian WHERE assignment_id=?");
        ...
        if ($ex) {
            ...
            $pdo->prepare("
                INSERT INTO reviewer_penilaian
        ...
        redirect('/modules/penelitian/reviewer.php' . ($submit ? '' : '?pid='.$asgn['prop_id']));
    }
}
...
if ($view_pid) {
    $vp = $pdo->prepare("
        ...
        JOIN reviewer_assignment ra ON ra.usulan_id=up.id AND ra.reviewer_id=?
        WHERE up.id=?
    ");
    ...
    if ($view_proposal) {
        ...
        $pn = $pdo->prepare("SELECT * FROM reviewer_penilaian WHERE assignment_id=?");
...
if ($view_pid && $view_proposal) {
    $vd = $pdo->prepare("SELECT deadline_review, catatan_admin FROM reviewer_assignment WHERE id=?");
...
$panduan_list = [];
try {
    $pq = $pdo->query("SELECT id, judul, deskripsi, file_path, file_name, link_url FROM panduan_reviewer WHERE is_active=1 ORDER BY urutan ASC, id ASC");
  ...
  <div class="main-content">
    ...
    <div class="topbar">
      ...
      <div class="topbar-right">
        <?php if ($view_proposal): ?>
        <a href="<?= BASE_URL ?>/modules/penelitian/reviewer.php" class="btn btn-outline" style="font-size:12px">
          ← <?= $id?'Kembali':'Back' ?>
        </a>
        <?php endif; ?>
