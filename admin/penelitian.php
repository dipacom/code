...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    // Toggle skema buka/tutup (via skema_penelitian table)
    if ($_POST['action'] === 'toggle_skema') {
        $skema_id = (int)($_POST['skema_id'] ?? 0);
        if ($skema_id) {
            $cur = (int)$pdo->query("SELECT is_open FROM skema_penelitian WHERE id=$skema_id")->fetchColumn();
            $pdo->prepare("UPDATE skema_penelitian SET is_open=? WHERE id=?")->execute([$cur ? 0 : 1, $skema_id]);
        }
        redirect('/admin/penelitian.php');
    }
    ...
    if ($_POST['action'] === 'review') {
        ...
        if ($pid && $status) {
            $pdo->prepare("
                UPDATE usulan_penelitian SET status=?, catatan_reviewer=?,
            ...
            if ($proposal) {
                $judul_notif = match($status) {
                    'disetujui' => $id?'Proposal Penelitian Disetujui':'Research Proposal Approved',
                    'direvisi'  => $id?'Proposal Perlu Direvisi':'Proposal Requires Revision',
                    'ditolak'   => $id?'Proposal Penelitian Ditolak':'Research Proposal Rejected',
                    default     => $id?'Status Proposal Diperbarui':'Proposal Status Updated',
                };
                $tipe = match($status) {
                    'disetujui' => 'sukses',
                    'direvisi'  => 'peringatan',
                ...
                $pesan = ($id?'Proposal "':'Proposal "').mb_strimwidth($proposal['judul'],0,80,'...')
...
$stats = $pdo->query("
    SELECT
      ...
      SUM(status IN ('seleksi_admin','lolos_admin')) n_tinjauan,
...
$deadline = getSetting($pdo,'penelitian_deadline') ?: '2026-05-18';
...
$statusBadge = fn($s) => match($s) {
    ...
    'seleksi_admin'            => ['bg'=>'#fef9c3','color'=>'#92400e', 'label_id'=>'Seleksi Admin',      'label_en'=>'Admin Review'],
...
<title><?= $id?'Usulan Penelitian':'Research Proposals' ?> (Admin) — LPPM</title>
...
.tgl-sw input:checked + .tgl-slider { background:#16a34a; }
...
.skema-tag.global   { background:#eff6ff; color:#1d4ed8; }
.skema-tag.nasional { background:#f0fdf4; color:#15803d; }
...
<div class="wrapper">
  ...
  <div class="main-content">
    ...
    <div class="topbar">
      <div>
        <div class="topbar-title">
          ...
          <?= $id?'Usulan Penelitian':'Research Proposals' ?>
    ...
    <div class="page-content">
      ...
      <div class="rpa-stats">
        <div class="rpa-stat">
          <div class="rpa-stat-num" style="color:var(--primary)"><?= $stats['total'] ?></div>
          <div class="rpa-stat-lbl"><?= $id?'Total Proposal':'Total Proposals' ?></div>
        </div>
        <div class="rpa-stat">
          <div class="rpa-stat-num" style="color:#2563eb"><?= $stats['n_baru'] ?></div>
          <div class="rpa-stat-lbl"><?= $id?'Diajukan':'Submitted' ?></div>
        </div>
        <div class="rpa-stat">
          <div class="rpa-stat-num" style="color:#ca8a04"><?= $stats['n_tinjauan'] ?></div>
          <div class="rpa-stat-lbl"><?= $id?'Ditinjau':'Under Review' ?></div>
        </div>
        <div class="rpa-stat">
          <div class="rpa-stat-num" style="color:#16a34a"><?= $stats['n_setuju'] ?></div>
          <div class="rpa-stat-lbl"><?= $id?'Disetujui':'Approved' ?></div>
        </div>
        <div class="rpa-stat">
          <div class="rpa-stat-num" style="color:#dc2626"><?= ($stats['n_tolak']+$stats['n_revisi']) ?></div>
      ...
      <div class="toggle-wrap">
        <span style="font-size:12.5px;font-weight:700;color:var(--text-primary)">
          <?= $id?'Penerimaan Proposal:':'Proposal Intake:' ?>
        </span>
        ...
        <form method="POST" style="display:inline-flex;align-items:center;gap:0">
          ...
          <div class="tgl-item">
            ...
            <span style="font-size:10.5px;font-weight:700;padding:2px 7px;border-radius:6px;
                         background:<?= $sk['is_open']?'#dcfce7':'#fee2e2' ?>;
                         color:<?= $sk['is_open']?'#15803d':'#dc2626' ?>">
        ...
        <span style="font-size:11.5px;color:var(--text-muted);margin-left:auto">
          ...
          &nbsp;·&nbsp;
          <a href="<?= BASE_URL ?>/admin/penelitian_setting.php" style="color:var(--primary);font-weight:600;text-decoration:none">
            <?= $id?'Pengaturan Skema →':'Scheme Settings →' ?>
          </a>
        </span>
      </div>
      ...
      <form method="GET" class="rpa-filters">
        ...
        <select name="skema" class="form-control" style="width:140px" onchange="this.form.submit()">
          <option value=""><?= $id?'Semua Skema':'All Schemes' ?></option>
          <option value="global"   <?= $filter_skema==='global'  ?'selected':'' ?>><?= $id?'Global (Scopus)':'Global (Scopus)' ?></option>
          <option value="nasional" <?= $filter_skema==='nasional'?'selected':'' ?>><?= $id?'Nasional (SINTA)':'National (SINTA)' ?></option>
        </select>
      ...
      <div class="card" style="padding:0;border-radius:12px;overflow:hidden">
        ...
        <div style="padding:48px 24px;text-align:center;color:var(--text-muted);font-size:13px">
          <?= ic('clipboard') ?>
          <div style="margin-top:10px"><?= $id?'Belum ada proposal.':'No proposals found.' ?></div>
        </div>
        <?php else: ?>
        ...
        <table class="rpa-table">
          <thead>
            <tr>
              <th>#</th>
              <th><?= $id?'Peneliti & Judul':'Researcher & Title' ?></th>
          ...
          <tr>
            ...
            <td>
              <span class="skema-tag <?= $p['skema'] ?>">
                <?= $p['skema']==='global'?'Global':'Nasional' ?>
              </span>
            </td>
...
<div class="drawer-overlay" id="drawerOverlay" onclick="closeDrawer(event)">
  <div class="drawer" id="drawer">
    ...
    <div class="drawer-body">
      ...
      <!-- Tim peneliti -->
      <div class="drawer-sec">
        <div class="drawer-sec-title"><?= $id?'Tim Peneliti':'Research Team' ?></div>
        <div id="dTeam"></div>
      </div>

      <!-- Self-check -->
      <div class="drawer-sec" id="dCheckSec" style="display:none">
        <div class="drawer-sec-title"><?= $id?'Self-Check Mandiri':'Self-Check' ?></div>
        <div id="dCheck"></div>
      </div>
      ...
      <div class="drawer-sec" id="dNoteSec" style="display:none">
        <div class="drawer-sec-title"><?= $id?'Catatan Reviewer':'Reviewer Notes' ?></div>
      ...
      <div class="drawer-sec" id="dReviewForm">
        ...
        <div class="form-group" style="margin-bottom:10px">
          <label class="form-label" style="font-size:12px"><?= $id?'Catatan untuk Dosen (opsional)':'Notes for Lecturer (optional)' ?></label>
...
function openDrawer(p) {
  ...
  document.getElementById('dSub').textContent =
    ...
    ' · ' + (p.skema === 'global' ? 'Global (Scopus)' : 'Nasional (SINTA)');

  // Info rows
  const sl = LANG==='id';
  const rows = [
    [sl?'Skema':'Scheme',     p.skema==='global'?'Publikasi Bereputasi Global':'Publikasi Bereputasi Nasional'],
    ...
    [sl?'Status':'Status',    '<span style="background:'+(statusMap[p.status]?'#eff6ff':'#f1f5f9')+';color:'+(p.status==='disetujui'?'#16a34a':p.status==='ditolak'?'#dc2626':p.status==='diajukan'?'#2563eb':'#64748b')+';padding:2px 9px;border-radius:6px;font-size:11px;font-weight:700">'+(statusMap[p.status]?statusMap[p.status][sl?'id':'en']:p.status)+'</span>'],
  ...
  teamHtml += `<div class="team-chip" style="background:var(--primary-xlight);border-color:var(--primary-light)">
    <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>
    <span style="font-weight:700">${p.nama_lengkap}</span>
  </div><br>`;
  // Anggota dosen
  let ad = [];
  ...
  // Self-check
  if (p.similarity_mandiri !== null || p.ai_mandiri !== null) {
    document.getElementById('dCheckSec').style.display = 'block';
    const simOk = parseFloat(p.similarity_mandiri) <= 25;
    const aiOk  = parseFloat(p.ai_mandiri) <= 30;
    document.getElementById('dCheck').innerHTML = `
      <div class="dl-row"><span class="dl-key">Similarity</span>
        <span class="dl-val" style="color:${simOk?'#16a34a':'#dc2626'}">
          ${p.similarity_mandiri !== null ? p.similarity_mandiri+'%' : '-'}
          ${!simOk?' ⚠ >25%':''}
        </span></div>
      <div class="dl-row"><span class="dl-key">${sl?'Deteksi AI':'AI Detection'}</span>
        <span class="dl-val" style="color:${aiOk?'#16a34a':'#dc2626'}">
          ${p.ai_mandiri !== null ? p.ai_mandiri+'%' : '-'}
          ${!aiOk?' ⚠ >30%':''}
        </span></div>
      <div class="dl-row"><span class="dl-key">${sl?'Platform Sim.':'Sim. Platform'}</span>
        <span class="dl-val">${p.platform_mandiri || '-'}</span></div>
      <div class="dl-row"><span class="dl-key">${sl?'Platform AI':'AI Platform'}</span>
        <span class="dl-val">${p.platform_ai_mandiri || '-'}</span></div>
    `;
  } else {
    document.getElementById('dCheckSec').style.display = 'none';
  }
