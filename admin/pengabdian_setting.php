...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
                ...
                $pdo->prepare("
                    INSERT INTO skema_pengabdian
                      (kode, nama, target_publikasi, anggaran_total, anggaran_penelitian,
                       ...
                       batas_luaran_bulan, jenis_luaran_wajib, kontrak_wajib)
                    VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
                ")->execute([
                    $kode,
                    $nama,
    ...
    if ($action === 'toggle_skema') {
        $sid = (int)($_POST['skema_id'] ?? 0);
        if ($sid) {
            $pdo->prepare("UPDATE skema_pengabdian SET is_open = 1-is_open WHERE id=?")
                ->execute([$sid]);
    ...
    if ($action === 'delete_skema') {
        ...
        if ($sid) {
            ...
            $kode = $pdo->query("SELECT kode FROM skema_pengabdian WHERE id=$sid")->fetchColumn();
            ...
            if ($n > 0) {
                ...
                $pdo->prepare("DELETE FROM skema_pengabdian WHERE id=?")->execute([$sid]);
...
// ── Load data ─────────────────────────────────────────────────
$settings = [];
$rows = $pdo->query("SELECT kunci,nilai FROM pengaturan WHERE kunci LIKE 'pengabdian%'")->fetchAll();
foreach ($rows as $r) $settings[$r['kunci']] = $r['nilai'];

$tahun       = $settings['pengabdian_tahun']            ?? date('Y');
$nomor_peng  = $settings['pengabdian_nomor_pengumuman'] ?? '';
$tgl_peng    = $settings['pengabdian_tgl_pengumuman']   ?? '';
$tgl_buka    = $settings['pengabdian_tgl_buka']         ?? '';
$deadline    = $settings['pengabdian_deadline']         ?? '';
$mandiri_buka= ($settings['pengabdian_mandiri_buka']    ?? '0') === '1';

$skemas = $pdo->query("SELECT * FROM skema_pengabdian ORDER BY urutan, id")->fetchAll();

$pernyataan_raw  = $settings['pengabdian_pernyataan_poin'] ?? null;
$pernyataan_poin = [];
if ($pernyataan_raw) {
...
if (empty($pernyataan_poin)) {
    $pernyataan_poin = [
        'Proposal merupakan usulan pengabdian baru (bukan lanjutan).',
        'Topik pengabdian relevan dengan kebutuhan masyarakat dan sesuai bidang keilmuan.',
        ...
        'Saya menyatakan bahwa semua informasi yang saya sampaikan adalah benar dan bersedia mematuhi seluruh aturan pengabdian LPPM IAKN Toraja.',
    ];
}
$pernyataan_text = implode("\n", $pernyataan_poin);

$template_raw  = $settings['pengabdian_template'] ?? null;
$template_list = [];
if ($template_raw) {
...
$edit_skema = null;
if (isset($_GET['edit'])) {
    $edit_skema = $pdo->prepare("SELECT * FROM skema_pengabdian WHERE id=?");
    $edit_skema->execute([(int)$_GET['edit']]);
    $edit_skema = $edit_skema->fetch();
}

$jabatan_opts = ['asisten_ahli'=>'Asisten Ahli (AA)', 'lektor'=>'Lektor', 'lektor_kepala'=>'Lektor Kepala', 'guru_besar'=>'Guru Besar / Profesor'];
?>
<!DOCTYPE html>
<html lang="<?= $lang ?>">
<head>
...
<title><?= $id?'Pengaturan PkM':'PkM Settings' ?> — LPPM Admin</title>
<link rel="icon" type="image/svg+xml" href="<?= BASE_URL ?>/assets/img/favicon.svg">
<link rel="stylesheet" href="<?= BASE_URL ?>/assets/css/style.css?v=4">
<style>
/* (Uses the same CSS framework as penelitian_setting.php) */
.cfg-sec { display:flex; align-items:center; gap:12px; margin:28px 0 16px; padding-bottom:10px; border-bottom:2px solid var(--border); }
.cfg-sec-ico { width:38px; height:38px; border-radius:10px; flex-shrink:0; display:flex; align-items:center; justify-content:center; }
...
.skema-card { border:1.5px solid var(--border); border-radius:12px; background:var(--bg-card); margin-bottom:11px; overflow:hidden; }
.skema-card.closed { opacity:.78; }
.skema-card-head { padding:12px 15px; display:flex; align-items:center; gap:11px; background:var(--bg-field); border-bottom:1px solid var(--border); }
.skema-card-ico { width:36px; height:36px; border-radius:8px; flex-shrink:0; display:flex; align-items:center; justify-content:center; }
.skema-card-ico svg { width:17px; height:17px; color:#fff; }
.skema-card-body { padding:12px 15px; }
.sk-strip { display:flex; flex-wrap:wrap; gap:7px 12px; align-items:center; font-size:12.5px; }
.sk-strip-item { display:inline-flex; align-items:center; gap:6px; padding:5px 10px; border-radius:7px; background:#f8fafc; border:1px solid #e2e8f0; }
...
.modal-overlay { position:fixed; inset:0; background:rgba(15,23,42,.6); z-index:900; display:none; align-items:center; justify-content:center; padding:16px; backdrop-filter:blur(4px); }
.modal-overlay.open { display:flex; }
.modal-box { background:var(--bg-card); border-radius:16px; width:100%; max-width:700px; max-height:92vh; overflow-y:auto; box-shadow:0 24px 60px rgba(0,0,0,.22); animation:slideUp .22s cubic-bezier(.22,1,.36,1); }
@keyframes slideUp { from{transform:translateY(20px);opacity:0} to{transform:translateY(0);opacity:1} }
.modal-head { padding:18px 20px 14px; border-bottom:1.5px solid var(--border); display:flex; align-items:center; gap:10px; }
.modal-body { padding:20px; }
.modal-foot { padding:14px 20px; border-top:1.5px solid var(--border); display:flex; gap:10px; justify-content:flex-end; }
...
.st-badge { display:inline-block; padding:3px 10px; border-radius:8px; font-size:11px; font-weight:700; }
.msec { border:1.5px solid var(--border); border-radius:11px; overflow:hidden; margin-bottom:13px; }
.msec-head { display:flex; align-items:center; gap:8px; padding:9px 14px; font-size:12px; font-weight:700; border-bottom:1.5px solid; }
  ...
  <div class="main-content">
    <div class="topbar">
      <div>
        <div class="topbar-title">
          <?= ic('settings') ?> <?= $id?'Pengaturan Pengabdian':'Community Service Settings' ?>
        </div>
      </div>
    ...
    <div class="page-content">
      ...
      <div class="cfg-sec">
        ...
        <div>
          <div class="cfg-sec-title">1. <?= $id?'Pengaturan Waktu & Mandiri':'Timeline & Independent Settings' ?></div>
          <div class="cfg-sec-sub"><?= $id?'Atur batas waktu pengajuan proposal pengabdian':'Set the community service proposal submission deadline' ?></div>
        </div>
      </div>
          ...
          <div style="padding:18px 20px 6px">
            <div class="form-grid-2" style="margin-bottom:14px">
              <div class="form-group" style="margin:0">
                <label class="form-label"><?= $id?'Nomor Pengumuman PkM':'PkM Announcement Number' ?></label>
                <input type="text" name="nomor_pengumuman" class="form-control" value="<?= htmlspecialchars($nomor_peng) ?>" placeholder="20/LPPM/IAKN-T/IV/2026">
              </div>
              <div class="form-group" style="margin:0">
                <label class="form-label"><?= $id?'Tahun Anggaran':'Budget Year' ?> <span class="required">*</span></label>
                <input type="number" name="tahun" class="form-control" min="2020" max="2099" value="<?= htmlspecialchars($tahun) ?>" required>
              </div>
            </div>
            <div class="form-grid-3" style="margin-bottom:14px">
              <div class="form-group" style="margin:0">
                <label class="form-label"><?= $id?'Tanggal Pengumuman':'Announcement Date' ?></label>
                <input type="date" name="tgl_pengumuman" class="form-control" value="<?= htmlspecialchars($tgl_peng) ?>">
              </div>
              <div class="form-group" style="margin:0">
                <label class="form-label"><?= $id?'Tanggal Dibuka':'Registration Opens' ?></label>
                <input type="date" name="tgl_buka" class="form-control" value="<?= htmlspecialchars($tgl_buka) ?>">
              </div>
              <div class="form-group" style="margin:0">
                <label class="form-label"><?= $id?'Batas Akhir Pengajuan':'Submission Deadline' ?> <span class="required">*</span></label>
                <input type="datetime-local" name="deadline" class="form-control" value="<?= htmlspecialchars(str_replace(' ', 'T', substr($deadline, 0, 16))) ?>" required>
              </div>
            </div>
            ...
            <div style="background:#f0f9ff;border:1px solid #bae6fd;border-radius:10px;padding:16px;margin-bottom:14px">
              <div style="display:flex;align-items:center;gap:8px;margin-bottom:8px">
                <div style="width:28px;height:28px;border-radius:6px;background:#0ea5e9;display:flex;align-items:center;justify-content:center;color:#fff">
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>
                </div>
                <div style="font-size:13.5px;font-weight:700;color:#0369a1"><?= $id?'Pengabdian Mandiri':'Independent Community Service' ?></div>
              </div>
              <label style="display:flex;align-items:center;gap:10px;cursor:pointer;font-size:13px;font-weight:600;color:#0c4a6e;margin-bottom:8px">
                <input type="checkbox" name="mandiri_buka" value="1" <?= $mandiri_buka?'checked':'' ?> style="width:16px;height:16px">
                <?= $id?'Buka Penerimaan Laporan Pengabdian Mandiri':'Open Independent Service Reporting' ?>
              </label>
              <p style="font-size:11.5px;color:#0284c7;line-height:1.6;margin:0">
                <?= $id?'<strong>Catatan:</strong> Struktur pengajuan dan pelaporan untuk skema Pengabdian Mandiri (yang didanai secara mandiri atau pihak ketiga) akan dikembangkan secara komprehensif pada pembaruan sistem berikutnya.':'<strong>Note:</strong> Submission and reporting structures for Independent Community Service will be fully developed in the next system update.' ?>
              </p>
            </div>
          </div>
          <div style="padding:12px 20px;background:var(--bg-field);border-top:1px solid var(--border)">
            <button type="submit" class="btn btn-primary" style="font-size:13px">
              <?= ic('check-circle') ?> <?= $id?'Simpan Pengaturan':'Save Settings' ?>
            </button>
          </div>
        </div>
      </form>

      <!-- ② Kelola Skema Kompetitif -->
      <div class="cfg-sec">
        <div class="cfg-sec-ico" style="background:linear-gradient(135deg,#047857,#10b981)">
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
            <path d="M2 3h6a4 4 0 0 1 4 4v14a3 3 0 0 0-3-3H2z"/><path d="M22 3h-6a4 4 0 0 0-4 4v14a3 3 0 0 1 3-3h7z"/>
          </svg>
        </div>
        <div style="flex:1">
          <div class="cfg-sec-title">2. <?= $id?'Kelola Skema Kompetitif':'Manage Competitive Schemes' ?></div>
          <div class="cfg-sec-sub"><?= $id?'Tambah, ubah, atau tutup skema PkM pendanaan kampus':'Manage campus-funded PkM schemes' ?></div>
        </div>
        <button type="button" class="btn btn-primary" style="font-size:12.5px;background:#059669;border:none" onclick="openAddModal()">
          <?= ic('plus') ?> <?= $id?'Tambah Skema':'Add Scheme' ?>
        </button>
      </div>

      <?php if (empty($skemas)): ?>
      <div style="background:var(--bg-card);border:1.5px dashed var(--border);border-radius:12px;padding:40px 24px;text-align:center;color:var(--text-muted)">
        <?= $id?'Belum ada skema. Klik "Tambah Skema" untuk membuat.':'No schemes yet. Click "Add Scheme" to create one.' ?>
      </div>
      <?php endif; ?>
      ...
      <div class="skema-card <?= $sk['is_open']?'':'closed' ?>">
        <div class="skema-card-head">
          <div class="skema-card-ico" style="background:linear-gradient(135deg,#064e3b,#059669)">
          ...
          <div style="flex:1;min-width:0">
            ...
            <div style="font-size:11.5px;color:var(--text-muted);margin-top:3px">
              <code><?= htmlspecialchars($sk['kode']) ?></code> · <?= $sk['tahun'] ?>
            </div>
          </div>
          <div style="display:flex;gap:6px;align-items:center;flex-shrink:0">
            <span class="st-badge <?= $sk['is_open']?'badge-open':'badge-closed' ?>"><?= $sk['is_open']?($id?'Buka':'Open'):($id?'Tutup':'Closed') ?></span>
            <form method="POST" style="margin:0"><input type="hidden" name="action" value="toggle_skema"><input type="hidden" name="skema_id" value="<?= $sk['id'] ?>"><button type="submit" class="btn btn-outline" style="font-size:11px;padding:5px 9px" title="<?= $sk['is_open']?($id?'Tutup skema':'Close scheme'):($id?'Buka skema':'Open scheme') ?>"><?= ic($sk['is_open']?'eye-off':'eye') ?></button></form>
            <button type="button" class="btn btn-outline" style="font-size:11px;padding:5px 9px" onclick="openEditModal(<?= htmlspecialchars(json_encode($sk, JSON_HEX_APOS|JSON_HEX_QUOT|JSON_HEX_AMP|JSON_HEX_TAG), ENT_QUOTES) ?>)"><?= ic('edit') ?></button>
            <form method="POST" style="margin:0" onsubmit="return confirm('<?= $id?'Hapus skema ini?':'Delete this scheme?' ?>')"><input type="hidden" name="action" value="delete_skema"><input type="hidden" name="skema_id" value="<?= $sk['id'] ?>"><button type="submit" class="btn btn-outline" style="font-size:11px;padding:5px 9px;border-color:#fecaca;color:#dc2626"><?= ic('trash') ?></button></form>
          </div>
        </div>
        <div class="skema-card-body">
          <div class="sk-strip">
            <span class="sk-strip-item"><span class="sk-strip-lbl"><?= $id?'Ketua':'Lead' ?>:</span><span class="sk-strip-val"><?= $jabatan_opts[$sk['jabatan_min']] ?? $sk['jabatan_min'] ?></span></span>
            <span class="sk-strip-item"><span class="sk-strip-lbl"><?= $id?'Total Dana':'Total Fund' ?>:</span><span class="sk-strip-val" style="color:#15803d">Rp <?= number_format($sk['anggaran_total'],0,',','.') ?></span></span>
            <span class="sk-strip-item"><span class="sk-strip-lbl"><?= $id?'Kuota':'Quota' ?>:</span><span class="sk-strip-val"><?= $sk['kuota'] ?></span></span>
            <span class="sk-strip-item"><span class="sk-strip-lbl"><?= $id?'Dosen':'Lect.' ?>:</span><span class="sk-strip-val"><?= $sk['max_anggota_dosen'] ? "{$sk['min_anggota_dosen']}–{$sk['max_anggota_dosen']}" : "≥{$sk['min_anggota_dosen']}" ?></span></span>
            <span class="sk-strip-item"><span class="sk-strip-lbl"><?= $id?'Mhs':'Std.' ?>:</span><span class="sk-strip-val"><?= $sk['max_anggota_mahasiswa'] ? "{$sk['min_anggota_mahasiswa']}–{$sk['max_anggota_mahasiswa']}" : "≥{$sk['min_anggota_mahasiswa']}" ?></span></span>
          </div>
        </div>
      </div>
      <?php endforeach; ?>
      ...
      <div class="cfg-sec">
        <div class="cfg-sec-ico" style="background:linear-gradient(135deg,#78350f,#d97706)"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg></div>
        <div>
          <div class="cfg-sec-title">3. <?= $id?'Pernyataan Kesanggupan':'Declaration of Commitment' ?></div>
          <div class="cfg-sec-sub"><?= $id?'Poin wajib centang pengusul':'Required checkbox points for proposers' ?></div>
        </div>
      </div>
      <form method="POST">
        ...
        <div class="card" style="padding:0;border-radius:12px;overflow:hidden">
          <div style="padding:18px 20px">
            <textarea name="poin" class="form-control" rows="6" style="resize:vertical;font-size:13px;line-height:1.8"><?= htmlspecialchars($pernyataan_text) ?></textarea>
            <div class="form-hint"><?= $id?'Satu poin per baris.':'One point per line.' ?></div>
          </div>
          <div style="padding:12px 20px;background:var(--bg-field);border-top:1px solid var(--border)">
            <button type="submit" class="btn btn-primary"><?= ic('check-circle') ?> <?= $id?'Simpan Pernyataan':'Save Declaration' ?></button>
          </div>
        </div>
      </form>
      ...
      <!-- ④ Template Proposal -->
      <div class="cfg-sec">
        <div class="cfg-sec-ico" style="background:linear-gradient(135deg,#1d4ed8,#3b82f6)"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg></div>
        <div>
          <div class="cfg-sec-title">4. <?= $id?'Template Proposal PkM':'PkM Proposal Template' ?></div>
        </div>
      </div>
      ...
      <div class="card" style="padding:0;border-radius:12px;overflow:hidden;margin-bottom:14px">
        ...
        <div style="padding:13px 18px;display:flex;align-items:center;gap:12px;border-bottom:1px solid var(--border)">
          <div style="flex:1">
            <div style="font-size:13px;font-weight:600"><?= htmlspecialchars($tmpl['judul']) ?></div>
            <div style="font-size:11px;color:var(--text-muted);margin-top:2px">Diunggah: <?= $tmpl['uploaded_at'] ?></div>
          </div>
          <form method="POST" style="margin:0" onsubmit="return confirm('Hapus template?')">
            <input type="hidden" name="action" value="delete_template"><input type="hidden" name="template_idx" value="<?= $ti ?>">
            <button type="submit" class="btn btn-outline" style="border-color:#fecaca;color:#dc2626"><?= ic('trash') ?></button>
          </form>
        </div>
        <?php endforeach; ?>
      </div>
      ...
      <form method="POST" enctype="multipart/form-data">
        ...
        <div class="card" style="padding:0;border-radius:12px;overflow:hidden">
          <div style="padding:18px 20px;display:grid;grid-template-columns:1fr 1fr;gap:14px">
            <div class="form-group" style="margin:0"><label class="form-label">Judul Template</label><input type="text" name="judul_template" class="form-control" required></div>
            <div class="form-group" style="margin:0"><label class="form-label">File (PDF/DOC)</label><input type="file" name="file_template" class="form-control" accept=".pdf,.doc,.docx" required></div>
          </div>
          <div style="padding:12px 20px;background:var(--bg-field);border-top:1px solid var(--border)">
            <button type="submit" class="btn btn-primary"><?= ic('upload') ?> Upload Template</button>
          </div>
        </div>
      </form>
...
<div class="modal-overlay" id="skemaModal" onclick="closeModalOutside(event)">
  <div class="modal-box">
    <div class="modal-head">
      <div style="flex:1">
        <div id="modalTitle" style="font-size:14px;font-weight:800;color:var(--text-primary)"><?= $id?'Tambah Skema PkM':'Add PkM Scheme' ?></div>
      </div>
      <button onclick="closeModal()" style="background:none;border:none;cursor:pointer;color:var(--text-muted);padding:4px"><?= ic('x','style="width:20px;height:20px"') ?></button>
    </div>
    <form method="POST" id="skemaForm">
      ...
      <div class="modal-body">
        <div class="msec">
          <div class="msec-head" style="background:#eff6ff;color:#1e40af;border-bottom-color:#bfdbfe">Identitas Skema</div>
          <div class="msec-body">
            <div class="form-grid-2">
              <div class="form-group" style="margin:0"><label class="form-label">Kode Skema *</label><input type="text" name="kode" id="fKode" class="form-control" required pattern="[a-z0-9_]+"></div>
              <div class="form-group" style="margin:0"><label class="form-label">Tahun *</label><input type="number" name="tahun_skema" id="fTahun" class="form-control" value="<?= $tahun ?>" required></div>
            </div>
            <div class="form-group"><label class="form-label">Nama Skema *</label><input type="text" name="nama" id="fNama" class="form-control" required></div>
            <div class="form-grid-2" style="margin-bottom:0">
              <div class="form-group" style="margin:0"><label class="form-label">Target Luaran</label><input type="text" name="target_luaran" id="fTarget" class="form-control"></div>
              <div class="form-group" style="margin:0"><label class="form-label">Kuota Proposal *</label><input type="number" name="kuota" id="fKuota" class="form-control" value="10" required></div>
            </div>
          </div>
        </div>
        <div class="msec">
          <div class="msec-head" style="background:#fffbeb;color:#92400e;border-bottom-color:#fde68a">Syarat & Anggaran</div>
          <div class="msec-body">
            <div class="form-group"><label class="form-label">Minimal Jabatan Ketua</label><select name="jabatan_min" id="fJabatan" class="form-control"><?php foreach($jabatan_opts as $v=>$l) echo "<option value=\"$v\">$l</option>"; ?></select></div>
            <div class="form-grid-3" style="margin-bottom:0">
              <div class="form-group" style="margin:0"><label class="form-label">Biaya PkM (Rp)</label><input type="number" name="anggaran_pengabdian" id="fAngPengabdian" class="form-control" value="0"></div>
              <div class="form-group" style="margin:0"><label class="form-label">Publikasi (Rp)</label><input type="number" name="anggaran_publikasi" id="fAngPublikasi" class="form-control" value="0"></div>
              <div class="form-group" style="margin:0"><label class="form-label">Total (Rp)</label><input type="number" name="anggaran_total" id="fAngTotal" class="form-control" value="0" readonly style="background:var(--bg-field)"></div>
            </div>
          </div>
        </div>
        <div class="msec">
          <div class="msec-head" style="background:#f0fdf4;color:#14532d;border-bottom-color:#bbf7d0">Ketentuan Tim</div>
          <div class="msec-body">
            <div class="form-grid-3" style="margin-bottom:12px">
              <div class="form-group" style="margin:0"><label class="form-label">Min. Dosen</label><input type="number" name="min_anggota_dosen" id="fMinDosen" class="form-control" value="0"></div>
              <div class="form-group" style="margin:0"><label class="form-label">Max. Dosen</label><input type="number" name="max_anggota_dosen" id="fMaxDosen" class="form-control" placeholder="∞"></div>
              <div class="form-group" style="margin:0"><label class="form-label">Sifat</label><select name="anggota_dosen_wajib" id="fDosenWajib" class="form-control"><option value="0">Opsional</option><option value="1">Wajib</option></select></div>
            </div>
            <div class="form-grid-3" style="margin-bottom:0">
              <div class="form-group" style="margin:0"><label class="form-label">Min. Mahasiswa</label><input type="number" name="min_anggota_mahasiswa" id="fMinMhs" class="form-control" value="2"></div>
              <div class="form-group" style="margin:0"><label class="form-label">Max. Mahasiswa</label><input type="number" name="max_anggota_mahasiswa" id="fMaxMhs" class="form-control" placeholder="∞"></div>
              <div class="form-group" style="margin:0"><label class="form-label">Sifat</label><select name="anggota_mahasiswa_wajib" id="fMhsWajib" class="form-control"><option value="1">Wajib</option><option value="0">Opsional</option></select></div>
            </div>
          </div>
        </div>
        <div class="msec">
          <div class="msec-head" style="background:var(--bg-field)">Lain-lain</div>
          <div class="msec-body">
            <div class="form-group"><label class="form-label">Deskripsi</label><textarea name="deskripsi" id="fDeskripsi" class="form-control" rows="2"></textarea></div>
            <label style="display:flex;align-items:center;gap:6px;font-size:12.5px;cursor:pointer"><input type="checkbox" name="is_open" id="fIsOpen" value="1" checked> Skema Langsung Dibuka</label>
          </div>
        </div>
      </div>
      <div class="modal-foot">
        <button type="button" class="btn btn-outline" onclick="closeModal()">Batal</button>
        <button type="submit" class="btn btn-primary" id="submitLbl">Simpan Skema</button>
      </div>
    </form>
  </div>
</div>
...
function openAddModal() {
  ...
  document.getElementById('modalTitle').textContent = 'Tambah Skema PkM';
  document.getElementById('submitLbl').textContent = 'Simpan Skema';
...
function openEditModal(sk) {
  ...
  document.getElementById('modalTitle').textContent = 'Edit Skema PkM';
  document.getElementById('submitLbl').textContent = 'Simpan Perubahan';
  
  document.getElementById('fKode').value = sk.kode; document.getElementById('fKode').disabled = true;
  document.getElementById('fTahun').value = sk.tahun;
  document.getElementById('fNama').value = sk.nama;
  document.getElementById('fTarget').value = sk.target_luaran || '';
  document.getElementById('fKuota').value = sk.kuota;
  document.getElementById('fJabatan').value = sk.jabatan_min;
  document.getElementById('fAngPengabdian').value = sk.anggaran_pengabdian;
  document.getElementById('fAngPublikasi').value = sk.anggaran_publikasi;
  document.getElementById('fMinDosen').value = sk.min_anggota_dosen;
  document.getElementById('fMaxDosen').value = sk.max_anggota_dosen || '';
  document.getElementById('fDosenWajib').value = sk.anggota_dosen_wajib ? '1' : '0';
  document.getElementById('fMinMhs').value = sk.min_anggota_mahasiswa;
  document.getElementById('fMaxMhs').value = sk.max_anggota_mahasiswa || '';
  document.getElementById('fMhsWajib').value = sk.anggota_mahasiswa_wajib ? '1' : '0';
...
function closeModal() { document.getElementById('skemaModal').classList.remove('open'); }
function closeModalOutside(e) { if (e.target === document.getElementById('skemaModal')) closeModal(); }
function autoSumTotal() {
  const p = parseInt(document.getElementById('fAngPengabdian').value) || 0;
  const pub = parseInt(document.getElementById('fAngPublikasi').value) || 0;
  document.getElementById('fAngTotal').value = p + pub;
}
document.getElementById('fAngPengabdian').addEventListener('input', autoSumTotal);
document.getElementById('fAngPublikasi').addEventListener('input', autoSumTotal);
...
$skemas = $pdo->query(
    "SELECT * FROM skema_pengabdian ORDER BY urutan, id"
)->fetchAll();
      ...
      <div class="modal-body">
        ...
        <div class="msec">
          <div class="msec-head" style="background:#fffbeb;color:#92400e;border-bottom-color:#fde68a">
            ...
            <?= $id?'Kontrak PkM — Opsional / Wajib':'PkM Contract — Optional / Required' ?>
          </div>
          <div class="msec-body">
            <label style="display:flex;align-items:flex-start;gap:11px;cursor:pointer;padding:10px;border:1.5px solid var(--border);border-radius:9px;background:var(--bg-field)">
              <input type="checkbox" name="kontrak_wajib" id="fKontrakWajib" value="1" style="margin-top:3px;width:16px;height:16px;flex-shrink:0">
              <div>
                <div style="font-weight:700;font-size:12.5px;color:var(--text-primary)">
                  <?= $id?'Kontrak PkM wajib untuk skema ini':'Contract required for this scheme' ?>
                </div>
                <div style="font-size:11.5px;color:var(--text-muted);margin-top:3px;line-height:1.55">
                  <?= $id
                    ? 'Jika dicentang: proposal yang disetujui WAJIB melalui penandatanganan kontrak sebelum dapat menyerahkan laporan & luaran. Jika tidak dicentang (default): proposal disetujui dapat langsung melaksanakan PkM tanpa kontrak formal — admin tetap dapat mengupload kontrak opsional kapan saja.'
                    : 'When checked: approved proposals MUST go through contract signing before report/output. When unchecked (default): approved proposals can proceed directly without formal contract — admin can still upload optional contract anytime.' ?>
                </div>
              </div>
            </label>
          </div>
...
function openAddModal() {
  ...
  const kw = document.getElementById('fKontrakWajib'); if (kw) kw.checked = false;
  // Show kode field
  document.getElementById('fKode').disabled = false;
...
function openEditModal(sk) {
  ...
  // Kontrak opsional/wajib (PkM)
  const kw = document.getElementById('fKontrakWajib');
  if (kw) kw.checked = (sk.kontrak_wajib == 1);
