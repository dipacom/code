...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($_POST['action'] === 'assign') {
        ...
        if ($pid && $reviewer_id) {
            ...
            $cnt = $pdo->prepare("SELECT COUNT(*) FROM reviewer_assignment WHERE usulan_id=? AND (deleted_at IS NULL OR deleted_at IS NULL)");
            $cnt->execute([$pid]);
            if ((int)$cnt->fetchColumn() >= 2) {
                ...
                try {
                    ...
                    $prop = $pdo->prepare("SELECT judul, status FROM usulan_penelitian WHERE id=?");
                    $prop->execute([$pid]);
                    $pdata = $prop->fetch();
                    if ($pdata && $pdata['status'] === 'lolos_admin') {
                        $pdo->prepare("UPDATE usulan_penelitian SET status='seleksi_substansi', updated_at=NOW() WHERE id=?")
                            ->execute([$pid]);
                    }
        ...
        redirect('/admin/penelitian_reviewer.php?pid=' . $pid);
    }
    ...
    if ($_POST['action'] === 'unassign') {
        ...
        if ($assign_id) {
            ...
            $has_sub = $pdo->prepare("SELECT COUNT(*) FROM reviewer_penilaian WHERE assignment_id=? AND submitted_at IS NOT NULL");
            ...
            if ((int)$has_sub->fetchColumn() > 0) {
                ...
                $pdo->prepare("DELETE FROM reviewer_assignment WHERE id=?")->execute([$assign_id]);
    ...
    if ($_POST['action'] === 'finalize') {
        ...
        if ($pid && $keputusan) {
            $pdo->prepare("
                ...
                WHERE id=?
            ")->execute([$keputusan, $catatan, $_SESSION['user_id'], $pid]);
    ...
    if ($_POST['action'] === 'review_revisi') {
        ...
        if ($pid && $revisi_id && $keputusan) {
            $prop = $pdo->prepare("SELECT user_id, judul, status FROM usulan_penelitian WHERE id=?");
            $prop->execute([$pid]);
            $pdata = $prop->fetch();
            ...
            if ($pdata) {
                if ($keputusan === 'diterima') {
                    ...
                    $prev_q = $pdo->prepare("SELECT status_sebelum FROM revisi_proposal WHERE id=?");
                ...
                $pdo->prepare("UPDATE revisi_proposal SET keputusan=?, admin_catatan=?, keputusan_at=NOW() WHERE id=?")
                    ->execute([$keputusan, $catatan, $revisi_id]);

                $pdo->prepare("UPDATE usulan_penelitian SET status=?, reviewed_by=?, reviewed_at=NOW() WHERE id=?")
        ...
        redirect('/admin/penelitian_reviewer.php?status=perbaikan_substantif');
    }
}
...
$proposals = $pdo->prepare("
    SELECT up.*, u.nama_lengkap,
           (SELECT COUNT(*) FROM reviewer_assignment WHERE usulan_id=up.id) as n_reviewer,
  ...
  <div class="main-content">
    ...
    <div class="topbar">
      ...
      <div class="topbar-right">
        <?php if ($view_proposal): ?>
        <a href="<?= BASE_URL ?>/admin/penelitian_reviewer.php" class="btn btn-outline" style="font-size:12px">
          ← <?= $id?'Kembali':'Back' ?>
        </a>
        <?php else: ?>
        ...
        <a href="<?= BASE_URL ?>/admin/penelitian_reviewer_laporan.php" target="_blank"
