...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'add' || $action === 'edit') {
        ...
        if (!$judul) {
            ...
            redirect('/admin/panduan_reviewer_pkm.php');
        }

        // Ambil file lama jika edit
        $old = null;
        if ($action === 'edit' && $pid) {
            $oq = $pdo->prepare("SELECT file_path, file_name, file_size FROM panduan_reviewer_pkm WHERE id=?");
            $oq->execute([$pid]);
            $old = $oq->fetch();
        }
        ...
        if (!empty($_FILES['file']['name'])) {
            ...
            $dir = BASE_PATH . '/uploads/panduan_reviewer_pkm/';
            ...
            if (move_uploaded_file($f['tmp_name'], $dir . $fname)) {
                ...
                $file_path = 'uploads/panduan_reviewer_pkm/' . $fname;
                $file_name = $f['name'];
                $file_size = $f['size'];
            }
        }
        ...
        if ($action === 'add') {
            $pdo->prepare("
                INSERT INTO panduan_reviewer_pkm
            ...
            $pdo->prepare("
                UPDATE panduan_reviewer_pkm SET
    ...
    if ($action === 'delete') {
        $pid = (int)($_POST['id'] ?? 0);
        $oq = $pdo->prepare("SELECT file_path FROM panduan_reviewer_pkm WHERE id=?");
        $oq->execute([$pid]);
        $row = $oq->fetch();
        if ($row && $row['file_path'] && file_exists(BASE_PATH . '/' . $row['file_path'])) {
            @unlink(BASE_PATH . '/' . $row['file_path']);
        }
        $pdo->prepare("DELETE FROM panduan_reviewer_pkm WHERE id=?")->execute([$pid]);
    ...
    if ($action === 'toggle') {
        $pid = (int)($_POST['id'] ?? 0);
        $pdo->prepare("UPDATE panduan_reviewer_pkm SET is_active = 1 - is_active WHERE id=?")->execute([$pid]);
        redirect('/admin/panduan_reviewer_pkm.php');
    }
}
