...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'add_proses') {
        ...
        if ($pid && $tahap && $tanggal && $catatan) {
            ...
            // Notifikasi peneliti
            $u = $pdo->prepare("SELECT user_id, judul FROM usulan_penelitian WHERE id=?");
            $u->execute([$pid]);
            if ($pd = $u->fetch()) {
                $pdo->prepare("INSERT INTO notifikasi (user_id,judul,pesan,tipe) VALUES (?,?,?,?)")
                    ->execute([$pd['user_id'],
                        $id?'Catatan Monev Penelitian':'Research Monev Note',
        ...
        redirect('/admin/penelitian_monev.php?pid=' . $pid . '&tab=proses');
    }
    ...
    if ($action === 'add_luaran' || $action === 'edit_luaran') {
        ...
        if ($pid && $jenis) {
            $up = $pdo->prepare("SELECT user_id FROM usulan_penelitian WHERE id=?");
            $up->execute([$pid]);
            $u = $up->fetch();
            if (!$u) redirect('/admin/penelitian_monev.php?pid=' . $pid);
        ...
        redirect('/admin/penelitian_monev.php?pid=' . $pid . '&tab=luaran');
    }
    ...
    if ($action === 'del_proses') {
        ...
        $pid = (int)($_POST['pid'] ?? 0);
        if ($mid) {
            $pdo->prepare("UPDATE monev_proses SET deleted_at=NOW() WHERE id=?")->execute([$mid]);
    ...
    if ($action === 'del_luaran') {
        ...
        $pid = (int)($_POST['pid'] ?? 0);
        if ($lid) {
            $pdo->prepare("UPDATE monev_luaran SET deleted_at=NOW() WHERE id=?")->execute([$lid]);
...
$proposals = $pdo->prepare("
    SELECT up.id, up.judul, up.skema, up.status, up.user_id, up.tahun_anggaran,
           ...
           (SELECT COUNT(*) FROM monev_proses mp WHERE mp.usulan_id = up.id AND mp.deleted_at IS NULL) AS n_proses,
           (SELECT COUNT(*) FROM monev_luaran ml WHERE ml.usulan_id = up.id AND ml.deleted_at IS NULL) AS n_luaran,
           (SELECT COUNT(*) FROM monev_luaran ml WHERE ml.usulan_id = up.id AND ml.status='diverifikasi' AND ml.deleted_at IS NULL) AS n_luaran_terbit
    FROM usulan_penelitian up
    JOIN users u ON up.user_id = u.id
    LEFT JOIN kontrak_penelitian kp ON kp.usulan_id = up.id
    LEFT JOIN skema_penelitian sk ON sk.kode COLLATE utf8mb4_unicode_ci = up.skema COLLATE utf8mb4_unicode_ci AND sk.tahun = up.tahun_anggaran
...
if ($view_pid) {
    ...
    if ($view_p) {
        $pq = $pdo->prepare("
            ...
            FROM monev_proses mp LEFT JOIN users u ON u.id = mp.admin_id
        ...
        $lq = $pdo->prepare("
            ...
            FROM monev_luaran ml LEFT JOIN users u ON u.id = ml.verified_by
...
<title><?= $id?'Monev Penelitian':'Research Monev' ?> — LPPM</title>
    ...
    <div class="topbar">
      <div>
        <div class="topbar-title">
          <?= ic('chart') ?>
          <?= $id?'Monev Penelitian':'Research Monev' ?>
          <span class="breadcrumb"><?= $tahun ?></span>
        </div>
      </div>
      <div class="topbar-right">
        <a href="<?= BASE_URL ?>/admin/penelitian.php" class="btn btn-outline" style="font-size:12px">← <?= $id?'Kembali':'Back' ?></a>
    ...
    <div class="page-content">
      ...
      <div style="background:#eff6ff;border:1.5px solid #bfdbfe;border-radius:11px;padding:13px 16px;margin-bottom:16px;font-size:12.5px;color:#1e40af">
        ...
        <span style="margin-left:5px"><?= $id
          ? 'Halaman ini melacak <strong>monitoring proses pelaksanaan</strong> dan <strong>luaran (publikasi)</strong> tiap proposal yang telah disetujui. Batas akhir luaran dihitung otomatis dari deadline laporan + batas bulan luaran skema (lihat <a href="' . BASE_URL . '/admin/penelitian_setting.php" style="color:#1e40af;text-decoration:underline">Pengaturan Penelitian</a>).'
      ...
      <a href="<?= BASE_URL ?>/admin/penelitian_monev.php" style="font-size:12px;color:var(--text-muted);text-decoration:none;display:inline-flex;align-items:center;gap:5px;margin-bottom:11px">
