...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'set_deadline') {
        ...
        if ($pid && $deadline) {
            ...
            $ck = $pdo->prepare("SELECT id FROM kontrak_pengabdian WHERE usulan_id=?");
            $ck->execute([$pid]);
            if ($ck->fetch()) {
                $pdo->prepare("UPDATE kontrak_pengabdian SET deadline_laporan=? WHERE usulan_id=?")
                    ->execute([$deadline, $pid]);
            } else {
                $pdo->prepare("INSERT INTO kontrak_pengabdian (usulan_id,deadline_laporan) VALUES (?,?)")
            ...
            if ($pdata) {
                $pdo->prepare("INSERT INTO notifikasi (user_id,judul,pesan,tipe) VALUES (?,?,?,?)")
                    ->execute([$pdata['user_id'],
                        $id?'Deadline Laporan Pengabdian Ditetapkan':'PkM Report Deadline Set',
                        ($id?'Deadline penyerahan laporan pengabdian Anda ditetapkan pada: ':'Your PkM report deadline has been set to: ')
                        . date('d F Y', strtotime($deadline)),
                        'info']);
            }
            $_SESSION['flash'] = ['type'=>'success','msg'=>$id?'Deadline berhasil disimpan.':'Deadline saved.'];
        }
        redirect('/admin/pengabdian_laporan.php');
    }
    ...
    if ($action === 'review_laporan') {
        ...
        if ($lap_id && $keputusan) {
            ...
            $cur = $pdo->prepare("SELECT lp.*, up.id AS usulan_id, up.judul, up.user_id AS pengusul_id
                                  FROM laporan_pengabdian lp
                                  JOIN usulan_pengabdian up ON up.id = lp.usulan_id
                                  WHERE lp.id = ?");
            $cur->execute([$lap_id]);
            $cur = $cur->fetch();
            if (!$cur) { redirect('/admin/pengabdian_laporan.php'); }

            $round = (int)($cur['round_ke'] ?? 1);

            // Catat history (snapshot file + keputusan)
            $pdo->prepare("
                INSERT INTO laporan_pengabdian_revisi
            ...
            if ($keputusan === 'diterima') {
                $pdo->prepare("UPDATE usulan_pengabdian SET status='laporan_diterima', updated_at=NOW()
            ...
            // Notifikasi ke pengusul
            $jdl = match($keputusan) {
                'diterima' => $id?'Laporan Pengabdian Diterima':'PkM Report Accepted',
                'revisi'   => $id?'Laporan Perlu Direvisi':'PkM Report Needs Revision',
                'ditolak'  => $id?'Laporan Pengabdian Ditolak':'PkM Report Rejected',
            };
            $msg = match($keputusan) {
                'diterima' => $id?'Laporan pengabdian Anda telah diterima oleh LPPM. ':'Your PkM report has been accepted. ',
    ...
    if ($action === 'tambah_sanksi') {
        ...
        if ($pid && $deskripsi) {
            $u_q = $pdo->prepare("SELECT user_id, judul FROM usulan_pengabdian WHERE id=?");
            $u_q->execute([$pid]);
            $u = $u_q->fetch();
            if ($u) {
                $berlaku_sd = $durasi > 0 ? date('Y-m-d', strtotime("+{$durasi} years")) : null;
                $pdo->prepare("
                    INSERT INTO monev_pengabdian_sanksi
                ...
                $pdo->prepare("INSERT INTO notifikasi (user_id,judul,pesan,tipe) VALUES (?,?,?,?)")
                    ->execute([$u['user_id'],
                        $id?'Sanksi Laporan Pengabdian':'PkM Report Sanction',
        ...
        redirect('/admin/pengabdian_laporan.php?pid=' . $pid);
    }
}
...
$proposals = $pdo->prepare("
    SELECT up.id, up.judul, up.skema, up.status, up.reviewed_at, up.user_id,
           ...
           (SELECT COUNT(*) FROM laporan_pengabdian_revisi lr WHERE lr.usulan_id = up.id) AS n_history,
           (SELECT COUNT(*) FROM monev_pengabdian_sanksi ms WHERE ms.usulan_id = up.id AND ms.status='aktif' AND ms.deleted_at IS NULL) AS n_sanksi
...
if ($view_pid) {
    ...
    $sq = $pdo->prepare("
        ...
        FROM monev_pengabdian_sanksi s
...
<title><?= $id?'Laporan Pengabdian':'PkM Report' ?> — LPPM Admin</title>
    ...
    <div class="topbar">
      <div>
        <div class="topbar-title">
          ...
          <?= $id?'Laporan Pengabdian':'PkM Reports' ?>
      ...
      <div class="topbar-right">
        ...
        <a href="<?= BASE_URL ?>/admin/pengabdian_laporan_cetak.php?tahun=<?= $tahun ?>" target="_blank"
    ...
    <div class="page-content">
      ...
      <div class="card" style="margin-bottom:18px;padding:0;overflow:hidden;border-color:#cbd5e1">
        <div style="padding:12px 16px;background:linear-gradient(135deg,#1e3a8a,#3b82f6);color:#fff;display:flex;align-items:center;gap:10px">
          ...
          <a href="<?= BASE_URL ?>/admin/pengabdian_laporan.php" style="color:#fff;background:rgba(255,255,255,.18);padding:5px 10px;border-radius:7px;font-size:11.5px;text-decoration:none">
            ✕ <?= $id?'Tutup':'Close' ?>
          </a>
        </div>
