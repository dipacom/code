...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($_POST['action'] === 'upload_kontrak') {
        ...
        if (!$pid) {
            ...
            redirect('/admin/pengabdian_kontrak.php');
        }

        // Kontrak hanya untuk proposal yang telah DISETUJUI di seleksi substantif
        $p_check = $pdo->prepare("SELECT id, status, user_id, judul FROM usulan_pengabdian WHERE id=? AND tahun_anggaran=?");
        ...
        if (!empty($_FILES['file_kontrak']['name'])) {
            ...
            $dir = BASE_PATH . '/uploads/kontrak_pengabdian/';
            if (!is_dir($dir)) mkdir($dir, 0755, true);
            $fname = time() . '_kontrak_' . $pid . '_' . preg_replace('/[^a-zA-Z0-9_.]/', '_', $f['name']);
            if (move_uploaded_file($f['tmp_name'], $dir . $fname)) {
                $file_kontrak      = 'uploads/kontrak_pengabdian/' . $fname;
        ...
        // Upsert kontrak
        $existing = $pdo->prepare("SELECT id, file_kontrak FROM kontrak_pengabdian WHERE usulan_id=?");
        ...
        if ($ex) {
            ...
            $pdo->prepare("
                INSERT INTO kontrak_pengabdian (usulan_id, nomor_kontrak, tgl_kontrak, deadline_laporan, file_kontrak, file_kontrak_name, admin_id, uploaded_at)
                VALUES (?,?,?,?,?,?,?,NOW())
            ")->execute([
                $pid,
                $nomor_kontrak ?: null,
        ...
        if ($prop['status'] === 'disetujui') {
            $pdo->prepare("UPDATE usulan_pengabdian SET status='penandatanganan_kontrak', updated_at=NOW() WHERE id=?")
                ->execute([$pid]);
        }

        // Notifikasi ke dosen
        $jdl_notif = $id?'Kontrak Pengabdian Siap':'PkM Contract Ready';
        $msg_notif = $id
            ? 'Kontrak pengabdian Anda telah disiapkan oleh LPPM. Silakan login untuk melihat dan mengunduh kontrak, kemudian serahkan kontrak yang sudah ditandatangani ke kantor LPPM.'
            : 'Your PkM contract is ready. Please log in to view and download it, then submit the signed version to the LPPM office.';
    ...
    if ($_POST['action'] === 'aktivasi_kontrak') {
        ...
        $pdo->prepare("UPDATE usulan_pengabdian SET status='kontrak_aktif', updated_at=NOW() WHERE id=?")
            ->execute([$pid]);

        $pdo->prepare("INSERT INTO notifikasi (user_id,judul,pesan,tipe) VALUES (?,?,?,?)")
            ->execute([$prop['user_id'],
                $id?'Kontrak Pengabdian Aktif':'PkM Contract Active',
                $id?'Kontrak pengabdian Anda kini aktif. Silakan mulai pelaksanaan pengabdian sesuai dengan ketentuan kontrak.'
                   :'Your PkM contract is now active. Please begin research according to the contract terms.',
                'sukses']);

        $_SESSION['flash'] = ['type'=>'success','msg'=>$id?'Kontrak diaktifkan. Proposal memasuki tahap pelaksanaan.':'Contract activated.'];
        redirect('/admin/pengabdian_kontrak.php?status=kontrak_aktif');
    }
}
...
$proposals = $pdo->prepare("
    ...
    LEFT JOIN kontrak_pengabdian kp ON kp.usulan_id = up.id AND (kp.deleted_at IS NULL OR kp.deleted_at IS NULL)
...
// Count per status
$cnt_disetujui = $pdo->query("SELECT COUNT(*) FROM usulan_pengabdian WHERE deleted_at IS NULL AND tahun_anggaran=$tahun AND status='disetujui'")->fetchColumn();
$cnt_kontrak   = $pdo->query("SELECT COUNT(*) FROM usulan_pengabdian WHERE deleted_at IS NULL AND tahun_anggaran=$tahun AND status='penandatanganan_kontrak'")->fetchColumn();
$cnt_aktif     = $pdo->query("SELECT COUNT(*) FROM usulan_pengabdian WHERE deleted_at IS NULL AND tahun_anggaran=$tahun AND status='kontrak_aktif'")->fetchColumn();
?>
<!DOCTYPE html>
<html lang="<?= $lang ?>">
<head>
...
<title>Kontrak Pengabdian — LPPM</title>
    ...
    <div class="topbar">
      <div>
        <div class="topbar-title">
          <?= ic('doc') ?>
          <?= $id?'Kontrak Pengabdian':'PkM Contracts' ?>
    ...
    <div class="page-content">
      ...
      <div style="background:#eff6ff;border:1.5px solid #bfdbfe;border-radius:12px;padding:14px 16px;margin-bottom:18px;font-size:12.5px;color:#1e40af">
        ...
        <div style="display:inline">
          <?= $id
            ? 'Kontrak pengabdian hanya dibuat untuk proposal yang telah <b>disetujui</b> pada tahap seleksi substantif. Alur: <b>Disetujui</b> → admin upload kontrak → <b>Penandatanganan</b> (peneliti datang ke LPPM menandatangani kontrak) → admin aktivasi → <b>Kontrak Aktif</b> (pengabdian dilaksanakan).'
...
<div class="modal-overlay" id="uploadModal" onclick="if(event.target===this)closeUpload()">
  <div class="modal-box">
    <div class="modal-head">
      ...
      <div>
        <div style="font-size:14px;font-weight:700"><?= $id?'Upload Kontrak Pengabdian':'Upload PkM Contract' ?></div>
      ...
      <div class="modal-body">
        ...
        <div class="form-group" style="margin-bottom:14px">
          <label class="form-label">
            <?= $id?'Deadline Laporan Pengabdian':'PkM Report Deadline' ?>
          ...
          <div style="font-size:11px;color:var(--text-muted);margin-top:4px">
            <?= $id?'Batas akhir pengusul mengirim laporan hasil pengabdian. Akan tampil pada dashboard peneliti.':'Deadline for proposer to submit PkM report. Displayed on researcher dashboard.' ?>
          </div>
        </div>
